...
# Cached passphrase for the lifetime of this process so we only prompt once
# when the bot has to decrypt multiple files (e.g. config.json + cookies.txt).
...
def decrypt_bytes(blob: str, passphrase: str) -> bytes:
    ...
    parts = blob.split("$", 3)
    # parts == ["", "BR1", "<salt_hex>", "<fernet_token>"]
    if len(parts) != 4:
...
def encrypt_path(plain_path: Union[str, Path], enc_path: Union[str, Path], passphrase: str) -> Path:
    ...
    try:
        ...
        # On Windows chmod is a no-op for most modes; ignore failures.
        pass
    return enc_path
...
def get_passphrase(prompt: str = "Passphrase: ", confirm: bool = False) -> str:
    ...
    Resolution order:
      1. ``BRAINROT_PASSPHRASE`` environment variable (for unattended runs).
      2. Cached value from an earlier prompt in this process.
      3. Interactive ``getpass`` prompt.

    When ``confirm`` is True the user is asked to type the passphrase twice.
    """
